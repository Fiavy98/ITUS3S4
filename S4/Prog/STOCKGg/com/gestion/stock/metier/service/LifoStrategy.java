...
public class LifoStrategy implements ValorizationStrategy {
    ...
    private ResultatSortie traiterSortie(int articleId, BigDecimal quantite, Connection conn, boolean applyUpdate)
        ...
        try {
            List<Lot> lots = lotRepository.findActiveLotsByArticle(articleId, "date_entree DESC, id DESC", conn);
            if (lots.isEmpty()) {
            ...
            throw new StockInsuffisantException("Erreur lors du calcul LIFO: " + e.getMessage(), e);
        }
        return result;
    }
}