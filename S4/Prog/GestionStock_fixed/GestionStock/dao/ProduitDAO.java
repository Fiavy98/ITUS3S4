...
public Produit getById(int id) throws Exception {
    ...
    String sql = """
        SELECT id, nom, methode_valuation, prix_vente_defaut, unite
        FROM produits
        WHERE id = ?
    """;
    ...
    try (Connection conn = ConnectionDB.getConnection();
        ...
        if (rs.next()) {
            ...
            p.setNom(rs.getString("nom"));
            p.setMethodeValuation(
                    MethodeValuation.valueOf(rs.getString("methode_valuation"))
            );
            p.setPrixVenteDefaut(rs.getDouble("prix_vente_defaut"));
            p.setUnite(rs.getString("unite"));
