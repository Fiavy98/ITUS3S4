...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        title = QLabel("🪫 Gestion d'Énergie Solaire")
        title.setObjectName("titleLabel")
        subtitle = QLabel("Configuration des panneaux solaires et calcul des économies d'énergie")
        ...
        self.tab_widget.addTab(self._build_config_tab(), "⚙️ Configuration")

        # Onglet Matériaux
        self.tab_widget.addTab(self._build_materials_tab(), "🔌 Appareils")

        # Onglet Résultats
        self.tab_widget.addTab(self._build_results_tab(), "📊 Résultats")
        ...
        self.calculate_btn = QPushButton("🚀 Lancer Simulation")
        ...
        self.db_status_label = QLabel("Base de données: Non connectée")
        self.db_status_label.setObjectName("dbStatusLabel")
        actions_layout.addWidget(self.db_status_label)

        root.addLayout(actions_layout)

    def _build_config_tab(self) -> QWidget:
        """Onglet Configuration - Paramètres du système solaire"""
        widget = QWidget()
        layout = QHBoxLayout(widget)
        layout.setSpacing(20)

        # Colonne gauche: Périodes horaires
        left_column = QVBoxLayout()
        left_column.addWidget(self._build_time_periods_group())
        left_column.addStretch(1)

        # Colonne droite: Paramètres techniques
        right_column = QVBoxLayout()
        right_column.addWidget(self._build_solar_panel_group())
        right_column.addWidget(self._build_economic_group())
    ...
    def _build_materials_tab(self) -> QWidget:
        """Onglet Matériaux - Gestion des appareils électriques"""
        widget = QWidget()
        layout = QVBoxLayout(widget)

        # Section de chargement depuis base
        load_section = QGroupBox("📥 Charger depuis Base de Données")
        load_layout = QHBoxLayout(load_section)

        load_layout.addWidget(QLabel("ID Simulation:"))
        self.simulation_id_input = QLineEdit("")
        self.simulation_id_input.setPlaceholderText("Ex: 24")
        self.simulation_id_input.setMaximumWidth(100)
        load_layout.addWidget(self.simulation_id_input)

        self.load_from_db_btn = QPushButton("🔍 Charger")
        self.load_from_db_btn.clicked.connect(self.on_load_devices_from_db)
        load_layout.addWidget(self.load_from_db_btn)

        load_layout.addStretch(1)
        layout.addWidget(load_section)

        # Section principale: Table des appareils
        devices_section = QGroupBox("🔌 Configuration des Appareils Électriques")
        devices_layout = QVBoxLayout(devices_section)

        # Description
        desc = QLabel("Ajoutez tous vos appareils électriques avec leur consommation et horaires d'utilisation.")
        desc.setObjectName("descriptionLabel")
        devices_layout.addWidget(desc)

        # Table des appareils
        self.devices_table = QTableWidget(0, 5)
        self.devices_table.setHorizontalHeaderLabels(
            ["📱 Appareil", "⚡ Puissance", "Unité", "🕐 Début", "🕐 Fin"]
        )
        self.devices_table.setAlternatingRowColors(True)
        self.devices_table.verticalHeader().setVisible(False)

        header = self.devices_table.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.Stretch)  # Nom appareil
        header.setSectionResizeMode(1, QHeaderView.ResizeToContents)  # Puissance
        header.setSectionResizeMode(2, QHeaderView.ResizeToContents)  # Unité
        header.setSectionResizeMode(3, QHeaderView.ResizeToContents)  # Début
        header.setSectionResizeMode(4, QHeaderView.ResizeToContents)  # Fin

        self.devices_table.setMinimumHeight(400)
        ...
        sample_btn = QPushButton("📋 Exemples")
        sample_btn.setObjectName("secondaryButton")
        sample_btn.clicked.connect(self.fill_sample)
        actions_layout.addWidget(sample_btn)

        clear_btn = QPushButton("🗑️ Vider Tout")
    ...
    def _build_results_tab(self) -> QWidget:
        """Onglet Résultats - Affichage des calculs et économies"""
        widget = QWidget()
        layout = QVBoxLayout(widget)

        # Section des résultats principaux
        results_section = QGroupBox("📊 Résultats de Simulation")
        ...
        self.res_panel_theoretical = self._build_result_card(cards_layout, 0, 0, "Puissance Théorique Panneau", "W", "⚡")
        self.res_panel_practical = self._build_result_card(cards_layout, 0, 1, "Puissance Pratique P1", "W", "🔋")
        self.res_available_energy = self._build_result_card(cards_layout, 1, 0, "Énergie Disponible", "Wh", "☀️")
        self.res_day_peak = self._build_result_card(cards_layout, 1, 1, "Pic Consommation Jour", "W", "🌅")
        self.res_evening_peak = self._build_result_card(cards_layout, 2, 0, "Pic Consommation Soir", "W", "🌙")
        self.res_ordinary_revenue = self._build_result_card(cards_layout, 2, 1, "Gain Jour Ordinaire", "Ar", "💰")
        self.res_holiday_revenue = self._build_result_card(cards_layout, 3, 0, "Gain Jour Férié", "Ar", "🎉")

        results_layout.addLayout(cards_layout)

        # Section des économies
        savings_section = QGroupBox("💡 Analyse des Économies")
        savings_layout = QVBoxLayout(savings_section)

        self.savings_analysis = QTextEdit()
        self.savings_analysis.setReadOnly(True)
        self.savings_analysis.setMinimumHeight(150)
        self.savings_analysis.setPlainText("Lancez une simulation pour voir l'analyse détaillée des économies.")
        savings_layout.addWidget(self.savings_analysis)

        layout.addWidget(results_section)
        layout.addWidget(savings_section)
        layout.addStretch(1)
