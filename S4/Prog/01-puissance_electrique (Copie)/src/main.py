...
class ApplicationPuissance:
    ...
    def _build_ui(self):
        ...
        ttk.Label(form, text="Matériel").grid(row=0, column=0, sticky="w", padx=(0, 8))
        ttk.Label(form, text="Heure début (HH:MM)").grid(row=0, column=1, sticky="w", padx=(0, 8))
        ttk.Label(form, text="Heure fin (HH:MM)").grid(row=0, column=2, sticky="w", padx=(0, 8))
        ...
        self.dropdown.grid(row=1, column=0, sticky="w", padx=(0, 8), pady=(4, 0))

        self.entry_debut = ttk.Entry(form, width=12)
        self.entry_debut.grid(row=1, column=1, sticky="w", padx=(0, 8), pady=(4, 0))
        self.entry_debut.insert(0, "08:00")

        self.entry_fin = ttk.Entry(form, width=12)
        self.entry_fin.grid(row=1, column=2, sticky="w", padx=(0, 8), pady=(4, 0))
        self.entry_fin.insert(0, "10:00")

        btn_add = ttk.Button(form, text="Ajouter", command=self.ajouter_utilisation)
        btn_add.grid(row=1, column=3, sticky="w", padx=(4, 0), pady=(4, 0))

        btn_calc = ttk.Button(form, text="Calculer panneau/batterie", command=self.calculer_resultats)
        btn_calc.grid(row=1, column=4, sticky="w", padx=(8, 0), pady=(4, 0))

        table_frame = ttk.LabelFrame(container, text="Utilisations ajoutées", padding=8)
        ...
        self.table.column("materiel", width=250)
        ...
        btn_supprimer = ttk.Button(actions_frame, text="Supprimer la sélection", command=self.supprimer_selection)
        btn_supprimer.pack(side="left")

        btn_tout_supprimer = ttk.Button(actions_frame, text="Tout supprimer", command=self.tout_supprimer)
        btn_tout_supprimer.pack(side="left", padx=(8, 0))

        result_frame = ttk.LabelFrame(container, text="Résultats", padding=10)
        result_frame.pack(fill="x", pady=(10, 0))

        self.lbl_panneau = ttk.Label(result_frame, text="Panneau nécessaire : -")
        self.lbl_panneau.pack(anchor="w")

        self.lbl_batterie = ttk.Label(result_frame, text="Batterie nécessaire : -")
        self.lbl_batterie.pack(anchor="w", pady=(4, 0))
