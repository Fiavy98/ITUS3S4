...
public static IntervRoute[] getByRoute(String rn) {

    String sql = """
        SELECT id, nom, depart_km, arrive_km, pluie
        FROM intervroute
        WHERE rn = ?
        ORDER BY depart_km
    """;
    ...
    try (Connection c = OracleConnection.getConnection();
        ...
        ArrayList<IntervRoute> list = new ArrayList<>();
        ...
        return list.toArray(new IntervRoute[0]);

    } catch (Exception e) {
        e.printStackTrace();
        return new IntervRoute[0];
    }
}
