...
public class Stock extends JPanel{
    ...
    public Stock(MaFenetre frame){
        ...
        btnValide.addActionListener(e -> {
            try {
                // 1️⃣ Créer le mouvement parent si nécessaire
                if (IDMVT == null) {
                ...
                // 2️⃣ Insérer les mouvements enfant
                if (IDMVT != null && !lsMedicaments.getSelectedValuesList().isEmpty()) {
                    new ActionStock(IDMVT, lsMedicaments, entre, uniteIdMap, unite).actionPerformed(e);
                } 
            } catch (Exception ex) {
