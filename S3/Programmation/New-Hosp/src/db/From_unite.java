...
public class From_unite {
    ...
    public static double getDivisibilite(String idMed, String idUnit){
        double div = 0.0;
        String query = "SELECT DIVISIBILITE FROM AS_EQUIVALENCE WHERE IDPRODUIT=? AND IDUNITE=?";
        ...
        try (Connection conn = OracleConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    div = rs.getDouble("DIVISIBILITE");
                }
            }
    
        } catch (SQLException e) {
            System.out.println("❌ Erreur SQL DIV : " + e.getMessage());
        }
